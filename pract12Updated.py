...
class LoginApp:
    ...
    def createWidgets(self):
        ...
        entryPassword = Entry(
            ...
            show="*"  # "*" to display a star symbol for each character typed
        )
    ...
    def authenticate(self):
        ...
        if username not in self.loginDetails:
            ...
            self.mainFrame.configure(bg="red")  # background color = red for unknown username
        elif self.loginDetails[username] != password:
            self.message.set("Incorrect password.")
            self.mainFrame.configure(bg="yellow")  # background color = yellow for wrong password
        else:
            self.message.set("Login successful!")
            self.mainFrame.configure(bg="green")  # background color = green for successful login
