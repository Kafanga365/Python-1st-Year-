...
class Order:
    ...
    def upgradeToStuffedCrust(self, pizzaIndex):
        if pizzaIndex < len(self.pizzas):
            pizza = self.pizzas[pizzaIndex]
            if not isinstance(pizza, StuffedCrustPizza):
                stuffedCrustPizza = StuffedCrustPizza(pizza.getSize(), "cheese")
                stuffedCrustPizza.toppings = pizza.toppings
...
def test():
    ...
    # Update the size of the first pizza
    myOrder.updatePizzaSize(0, "medium")

    # Update the toppings of the second pizza
    myOrder.updatePizzaToppings(1, ["Pepperoni", "Onions"])

    print(myOrder)

    # Upgrade the first pizza to stuffed crust
