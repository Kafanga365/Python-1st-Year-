...
class Laptop:
    ...
    def setSsd(self, capacity):
        if capacity == 256 or capacity == 512 or capacity == 1024:
            self.ssd = capacity
        else:
            print("Invalid SSD capacity.")

    def __str__(self):
        return f"Laptop with {self.ram} GB RAM and {self.ssd} GB SSD"


def testLaptopFunction():
    laptop = Laptop(8)  # laptop instance with 8 GB RAM
    print(laptop)  

    laptop.setSsd(512)  # Changes the SSD capacity to 512 GB from 256
    print(laptop)  
